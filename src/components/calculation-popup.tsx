...
export default function CalculationPopup({ missile, boost, onClose }: CalculationPopupProps) {
  ...
  const getCalculationSteps = (m: MissileType): CalculationStep[] => {
    ...
    return [
      ...
      {
        text: `Earth_New_Spin = Current_Spin + (${m.boost} × DeepSeek_Inspiration_Factor)`,
        textEn: `Earth_New_Spin = Current_Spin + (${m.boost} × DeepSeek_Inspiration_Factor)`,
        delay: 1200,
        type: 'formula',
      },
      {
  ...
  const steps = missile ? getCalculationSteps(missile) : [];
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
      <div 
        className={`
          relative w-full max-w-md bg-slate-900/95 border rounded-2xl shadow-2xl
              ...
              className={`
                ...
                ${step.type === 'formula' 
                  ...
                  : step.type === 'result'
                    ...
                    : 'bg-slate-800/30'}
                ${step.type === 'result' ? 'animate-pulse' : ''}
              `}
              style={{
